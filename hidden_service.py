...
def start_tor_process(config: Dict[str, str]) -> subprocess.Popen:
    tor_process = launch_tor_with_config(config=config)

    return tor_process


def run_hidden_service(port: int, key_path: str) -> None:
    ...
    with Controller.from_port(port=port) as controller:
        ...
        try:
            uvicorn.run("app:app", **hidden_service_server_data)
        finally:
